...
export default function Signup() {
    ...
    const handleSubmit = (e) => {
        e.preventDefault(); try { const response = await fetch("https://cinetrack-production.up.railway.app/auth/signup", { method: "POST", headers: { "Content-Type": "application/json", }, body: JSON.stringify(signupData), }); const data = await response.json(); if (response.ok) { toast.success(data.message || "Registration successful! Please check your email."); setFirstName(""); setUserName(""); setUserEmail(""); setUserPass(""); setConfirmPass(""); } else { toast.error(data.error || "Signup failed."); } } catch (error) { toast.error("Could not connect to the server."); console.error("Fetch error:", error); }
    };
